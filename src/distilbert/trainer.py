"""Fine-tuning engine for DistilBERT on BANKING77 with validation Macro F1 model selection."""
...
from src.distilbert.dataset import create_data_loaders


@dataclass
class EpochMetric:
    """Evaluation metrics for a single training epoch."""
    epoch: int
    train_loss: float
    val_loss: float
    val_macro_f1: float
...
class TrainingSummary:
    """Summary of the complete fine-tuning experiment."""
    best_epoch: int
    best_macro_f1: float
...
def set_seed(seed: int) -> None:
    """Enforce reproducibility across random, numpy, and torch."""
    random.seed(seed)
    np.random.seed(seed)
...
def evaluate_epoch(
    ...
    """Run validation evaluation pass and return (val_loss, accuracy, macro_f1)."""
    model.eval()
    total_loss = 0.0
    all_preds: List[int] = []
...
def train_distilbert(
    ...
    """Fine-tune DistilBERT end-to-end on BANKING77 with best-model checkpointing based on Macro F1."""
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if val_macro_f1 > best_macro_f1:
            ...
            best_epoch = epoch
            # Keep copy of best state dict in CPU memory
